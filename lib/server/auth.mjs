...
export function authenticateBearer(header = "") {
  ...
  if (mode === "mock") {
    if (token === "demo-token") return { galleryId: "gal_demo", role: "GALLERY" };
    if (token === "demo-accountant-token") return { galleryId: "gal_demo", role: "ACCOUNTANT" };
    return null;
  }
  ...
  return galleryId ? { galleryId, role: payload.role || "GALLERY" } : null;
}
