...
import { readSessionCookie, verifySessionToken } from "./session.mjs";
...
export function authenticateBearer(header = "") {
  ...
  if (!secret) {
    if ((process.env.AUTH_MODE || "mock") === "mock") return null;
...
export function authenticateRequest(request) {
  ...
  if (authorization) return authenticateBearer(authorization);
