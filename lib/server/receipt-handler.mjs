...
export async function payoutReceiptHandler(request, context) {
  ...
  const html = `<!doctype html><html lang="tr"><meta charset="utf-8"><title>TaşıtPOS Transfer Dekontu</title><style>body{font:16px Arial;max-width:720px;margin:48px auto;color:#10213d}header{border-bottom:3px solid #12a6a6;padding-bottom:20px}dl{display:grid;grid-template-columns:220px 1fr;gap:12px;margin-top:32px}dt{font-weight:700}footer{margin-top:40px;color:#667085;font-size:12px}</style><header><h1>TaşıtPOS Transfer Dekontu</h1><p>Hakediş banka transfer kaydı</p></header><dl><dt>Transfer / İşlem No</dt><dd>${escapeHtml(transaction.id)}</dd><dt>Galeri</dt><dd>${escapeHtml(transaction.galeri_id)}</dd><dt>Plaka</dt><dd>${escapeHtml(transaction.plate)}</dd><dt>Net aktarılan tutar</dt><dd>${escapeHtml(amount)}</dd><dt>Transfer tarihi</dt><dd>${escapeHtml(date)}</dd><dt>Banka</dt><dd>${escapeHtml(bank)}</dd></dl><footer>Bu belge TaşıtPOS işlem kaydından dinamik olarak oluşturulmuştur.</footer></html>`;
